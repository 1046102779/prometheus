...
输入时间戳可以被[RFC3339](https://www.ietf.org/rfc/rfc3339.txt)格式或者Unix时间戳提供。输出时间戳以Unix时间戳的方式呈现。
...
<value>是一个查询结果数据，依赖于这个`resultType`格式, 不同的结果类型，则会有不同的结果数据格式。见[表达式查询结果格式](https://prometheus.io/docs/querying/api/#expression-query-result-formats)。
